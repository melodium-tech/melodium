...
impl Model {
    ...
    pub fn add_parameter(&mut self, name: &str) -> Result<Arc<RwLock<Parameter>>, LogicError> {
        ...
        if self.descriptor().base_model().expect("Designed model must have base model").parameters().contains_key(name) {
            let parameter = Parameter::new( &(self.auto_reference.upgrade().unwrap() as Arc<RwLock<dyn Scope>>),
                                            &self.descriptor().base_model().expect("Designed model must have base model").as_parameterized(),
                                            name
                                        );
    ...
    pub fn validate(&self) -> Result<(), LogicError> {
        ...
        let rc_base_model = self.descriptor().base_model().expect("Designed model must have base model");
        let unset_params: Vec<&ParameterDescriptor> = rc_base_model.parameters().iter().filter_map(
