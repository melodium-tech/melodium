...
/// ```
#[mel_treatment(
    input first Stream<u64>
    input second Stream<u64>
...
pub async fn trigger() {
    ...
    if let Ok(values) = stream.recv_u64().await {
        ...
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_u64(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_u64().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_u64(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<u64>
    output stream Stream<u64>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_u64().await {
        let _ = stream.send_one_u64(val).await;
    }
}
...
pub async fn merge() {
    while let Ok(select) = select.recv_one_bool().await {
        ...
        check!(value.send_one_u64(val).await)
    }
}
...
#[mel_treatment(
    ...
    output filled Stream<u64>
)]
pub async fn fill(value: u64) {
    while let Ok(pat) = pattern.recv_void().await {
        check!(filled.send_u64(vec![value.clone(); pat.len()]).await)
    }
}
...
#[mel_treatment(
    ...
    output emit Block<u64>
)]
pub async fn emit(value: u64) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_u64(value).await;
    }
}
