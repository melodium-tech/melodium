...
/// Trigger on `u16` stream start and end.
///
...
#[mel_treatment(
    ...
    output first Block<u16>
    output last Block<u16>
)]
pub async fn trigger() {
    let mut last_value = None;

    if let Ok(values) = stream.recv_u16().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_u16(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_u16().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_u16(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<u16>
    output stream Stream<u16>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_u16().await {
        let _ = stream.send_one_u16(val).await;
    }
}
...
#[mel_treatment(
    ...
    output filled Stream<u16>
)]
pub async fn fill(value: u16) {
    while let Ok(pat) = pattern.recv_void().await {
        check!(filled.send_u16(vec![value.clone(); pat.len()]).await)
    }
}
...
#[mel_treatment(
    ...
    input select Stream<bool>
    output accepted Stream<u16>
    output rejected Stream<u16>
)]
pub async fn filter() {
    let mut accepted_op = true;
    let mut rejected_op = true;

    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_u16(), select.recv_one_bool())
    {
        if select {
            if let Err(_) = accepted.send_one_u16(value).await {
            ...
            if let Err(_) = rejected.send_one_u16(value).await {
...
#[mel_treatment(
    ...
    output emit Block<u16>
)]
pub async fn emit(value: u16) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_u16(value).await;
    }
}
