...
pub async fn flatten() {
    'main: while let Ok(vectors) = vector.recv_vec_i128().await {
        for vec in vectors {
            check!('main, value.send_i128(vec).await)
        }
    }
}

/// Chain two streams of `Vec<i128>`.
/// 
/// 
/// ```mermaid
/// graph LR
...
/// ```
#[mel_treatment(
    input first Stream<Vec<i128>>
    input second Stream<Vec<i128>>
    output chained Stream<Vec<i128>>
)]
pub async fn chain() {

    while let Ok(vectors) = first.recv_vec_i128().await {

        check!(chained.send_vec_i128(vectors).await)
    }

    while let Ok(vectors) = second.recv_vec_i128().await {

        check!(chained.send_vec_i128(vectors).await)
    }
}

/// Merge two streams of `Vec<i128>`.
/// 
...
/// ```
#[mel_treatment(
    input a Stream<Vec<i128>>
    input b Stream<Vec<i128>>
...
pub async fn merge() {
    while let Ok(select) = select.recv_one_bool().await {
        let val;
        if select {
            if let Ok(v) = a.recv_one_vec_i128().await {
                val = v;
            }
            else {
                break;
            ...
            if let Ok(v) = b.recv_one_vec_i128().await {
                val = v;
            }
            else {
                break;
        ...
        check!(value.send_one_vec_i128(val).await)
    }
}

/// Filter a `Vec<i128>` stream according to `bool` stream.
/// 
...
#[mel_treatment(
    ...
    input select Stream<bool>
    output accepted Stream<Vec<i128>>
    output rejected Stream<Vec<i128>>
)]
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_vec_i128(), select.recv_one_bool()) {
        if select {
            if let Err(_) = accepted.send_one_vec_i128(value).await {
            ...
            if let Err(_) = rejected.send_one_vec_i128(value).await {
...
/// Trigger on `Vec<i128>` stream start and end.
/// 
...
#[mel_treatment(
    ...
    output first Block<Vec<i128>>
    output last Block<Vec<i128>>
)]
pub async fn trigger() {

    let mut last_value = None;

    if let Ok(values) = stream.recv_vec_i128().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_vec_i128(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_vec_i128().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_vec_i128(val).await;
    }
...
/// Stream a block `Vec<i128>` element.
/// 
/// ```mermaid
/// graph LR
///     T("stream()")
...
#[mel_treatment(
    ...
    output stream Stream<Vec<i128>>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_vec_i128().await {
        let _ = stream.send_one_vec_i128(val).await;
    }
}

/// Emit a block `Vec<i128>` value.
/// 
...
#[mel_treatment(
    ...
    output emit Block<Vec<i128>>
)]
pub async fn emit(value: Vec<i128>) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_vec_i128(value).await;
    }
}

/// Gives pattern of a `Vec<i128>` stream.
/// 
/// ```mermaid
/// graph LR
///     T("pattern()")
...
/// Fill a pattern stream with a `i128` value.
/// 
/// ```mermaid
/// graph LR
/// T("fill(value=🟧)")
...
#[mel_treatment(
    default value 0
    input pattern Stream<Vec<void>>
    output filled Stream<Vec<i128>>
)]
pub async fn fill(value: i128) {
    while let Ok(pat) = pattern.recv_vec_void().await {
        check!(filled.send_vec_i128(pat.into_iter().map(|p| vec![value.clone(); p.len()]).collect()).await)
    }
}
...
#[mel_treatment(
    ...
    output resized Stream<Vec<i128>>
)]
pub async fn resize(default: i128) {
    while let Ok(size) = size.recv_one_u64().await {
        if let Ok(mut vec) = vector.recv_one_vec_i128().await {
            vec.resize(size as usize, default.clone());
            check!(resized.send_one_vec_i128(vec).await);
        }
        else {
            break;
        }
