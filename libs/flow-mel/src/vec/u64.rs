...
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_vec_u64(), select.recv_one_bool()) {
        if select {
...
/// Trigger on `Vec<u64>` stream start and end.
/// 
...
/// Stream a block `Vec<u64>` element.
/// 
/// ```mermaid
/// graph LR
///     T("stream()")
...
/// Gives pattern of a `Vec<u64>` stream.
/// 
/// ```mermaid
/// graph LR
///     T("pattern()")
...
pub async fn fill(value: u64) {
    while let Ok(pat) = pattern.recv_vec_void().await {
        check!(filled.send_vec_u64(pat.into_iter().map(|p| vec![value.clone(); p.len()]).collect()).await)
    }
}
