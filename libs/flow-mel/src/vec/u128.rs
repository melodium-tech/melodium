...
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) =
        futures::join!(value.recv_one_vec_u128(), select.recv_one_bool())
    {
        if select {
...
/// Trigger on `Vec<u128>` stream start and end.
///
...
#[mel_treatment(
    ...
    output first Block<Vec<u128>>
    output last Block<Vec<u128>>
)]
pub async fn trigger() {
    let mut last_value = None;

    if let Ok(values) = stream.recv_vec_u128().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_vec_u128(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_vec_u128().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_vec_u128(val).await;
    }
...
/// Stream a block `Vec<u128>` element.
///
/// ```mermaid
/// graph LR
///     T("stream()")
...
pub async fn stream() {
    if let Ok(val) = block.recv_one_vec_u128().await {
        let _ = stream.send_one_vec_u128(val).await;
    }
}

/// Emit a block `Vec<u128>` value.
///
...
#[mel_treatment(
    ...
    output emit Block<Vec<u128>>
)]
pub async fn emit(value: Vec<u128>) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_vec_u128(value).await;
    }
}
