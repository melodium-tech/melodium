...
/// Trigger on `i32` stream start and end.
///
...
#[mel_treatment(
    ...
    output first Block<i32>
    output last Block<i32>
)]
pub async fn trigger() {
    let mut last_value = None;

    if let Ok(values) = stream.recv_i32().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_i32(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_i32().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_i32(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<i32>
    output stream Stream<i32>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_i32().await {
        let _ = stream.send_one_i32(val).await;
    }
}
...
#[mel_treatment(
    ...
    output filled Stream<i32>
)]
pub async fn fill(value: i32) {
    while let Ok(pat) = pattern.recv_void().await {
        check!(filled.send_i32(vec![value.clone(); pat.len()]).await)
    }
}

/// Filter a `i32` stream according to `bool` stream.
///
...
#[mel_treatment(
    ...
    input select Stream<bool>
    output accepted Stream<i32>
    output rejected Stream<i32>
)]
pub async fn filter() {
    let mut accepted_op = true;
    let mut rejected_op = true;

    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_i32(), select.recv_one_bool())
    {
        if select {
            if let Err(_) = accepted.send_one_i32(value).await {
            ...
            if let Err(_) = rejected.send_one_i32(value).await {
...
/// Fit a stream of `i32` into a pattern.
///
...
#[mel_treatment(
    ...
    output emit Block<i32>
)]
pub async fn emit(value: i32) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_i32(value).await;
    }
}
