use super::{Connection, ModelInstanciation, Scope, TreatmentInstanciation, IO};
...
use melodium_common::descriptor::{
    Collection, Entry, Identifier, Parameterized, Treatment as TreatmentTrait,
};
...
impl Treatment {
    ...
    pub fn add_model_instanciation(
        ...
        name: &str,
    ) -> Result<Arc<RwLock<ModelInstanciation>>, LogicError> {
        if let Some(Entry::Model(model_descriptor)) = self
            .collection
            .as_ref()
            .ok_or_else(|| LogicError::collection_undefined())?
    ...
    pub fn add_treatment(
        ...
        name: &str,
    ) -> Result<Arc<RwLock<TreatmentInstanciation>>, LogicError> {
        if let Some(Entry::Treatment(treatment_descriptor)) = self
            .collection
            .as_ref()
            .ok_or_else(|| LogicError::collection_undefined())?
    ...
    pub fn add_connection(
        ...
        if let Some(pos_rc_output_treatment) = self.treatments.get(output_treatment) {
            ...
            if let Some(pos_output) = rc_output_treatment
                .read()
                .unwrap()
                .descriptor()
                .outputs()
        ...
        if let Some(pos_rc_input_treatment) = self.treatments.get(input_treatment) {
            ...
            if let Some(pos_input) = rc_input_treatment
                .read()
                .unwrap()
                .descriptor()
                .inputs()
    ...
    pub fn add_self_connection(
        ...
        if input_self.matches_output(&output_self) {
            self.connections
                .push(Connection::new_self(input_self.name(), output_self.name()));
            Ok(())
        } else {
            return Err(LogicError::unexisting_connexion_type());
        }
