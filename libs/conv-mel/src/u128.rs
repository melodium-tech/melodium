...
pub async fn to_void() {
    while let Ok(values) = value.recv_u128().await {
        check!(iter.send_void(vec![(); values.len()]).await)
    }
}


/// Turns `u128` into `Vec<byte>`.
#[mel_function]
pub fn to_byte(value: u128) -> Vec<byte> {
    value.to_be_bytes().to_vec()
}

/// Turns `u128` stream into `byte` one.
/// 
/// Each `u128` gets converted into `Vec<byte>`, with each vector containing the `bytes`s of the former scalar `u128` it represents.
#[mel_treatment(
    input value Stream<u128>
...
/// Turns `u128` into `f32`.
/// 
/// This conversion is lossless, as any `u128` value can fit into a `f32`.
#[mel_function]
pub fn to_f32(value: u128) -> f32 {
    value as f32
}

/// Turns `u128` stream into `f32` one.
/// 
/// Each `u128` gets converted into `f32`.
/// This conversion is lossless, as any `u128` value can fit into a `f32`.
#[mel_treatment(
    input value Stream<u128>
...
/// Turns `u128` into `f64`.
/// 
/// This conversion is lossless, as any `u128` value can fit into a `f64`.
#[mel_function]
pub fn to_f64(value: u128) -> f64 {
    value as f64
}

/// Turns `u128` stream into `f64` one.
/// 
/// Each `u128` gets converted into `f64`.
/// This conversion is lossless, as any `u128` value can fit into a `f64`.
#[mel_treatment(
    input value Stream<u128>
...
/// 
#[mel_function]
pub fn to_u8(value: u128, truncate: bool, or_default: u8) -> u8 {
    if truncate {
        value as u8
    }
    else {
...
/// 
#[mel_function]
pub fn to_u16(value: u128, truncate: bool, or_default: u16) -> u16 {
    if truncate {
        value as u16
    }
    else {
...
/// 
#[mel_function]
pub fn to_u32(value: u128, truncate: bool, or_default: u32) -> u32 {
    if truncate {
        value as u32
    }
    else {
...
/// 
#[mel_function]
pub fn to_u64(value: u128, truncate: bool, or_default: u64) -> u64 {
    if truncate {
        value as u64
    }
    else {
...
/// 
#[mel_function]
pub fn to_i8(value: u128, truncate: bool, or_default: i8) -> i8 {
    if truncate {
        value as i8
    }
    else {
...
pub async fn to_i8(truncate: bool, or_default: i8) {
    ...
    if truncate {
        while let Ok(values) = value.recv_u128().await {
            check!(into.send_i8(values.into_iter().map(|val| val as i8).collect()).await)
        }
    }
    else {
        use std::convert::TryInto;
        while let Ok(values) = value.recv_u128().await {
            check!(into.send_i8(values.into_iter().map(|val|
                TryInto::<i8>::try_into(val).unwrap_or(or_default)
            ).collect()).await)
        }
    }
}
...
/// 
#[mel_function]
pub fn to_i16(value: u128, truncate: bool, or_default: i16) -> i16 {
    if truncate {
        value as i16
    }
    else {
...
pub async fn to_i16(truncate: bool, or_default: i16) {
    ...
    if truncate {
        ...
        use std::convert::TryInto;
        while let Ok(values) = value.recv_u128().await {
            check!(into.send_i16(values.into_iter().map(|val|
                TryInto::<i16>::try_into(val).unwrap_or(or_default)
            ).collect()).await)
        }
    }
}
...
/// 
#[mel_function]
pub fn to_i32(value: u128, truncate: bool, or_default: i32) -> i32 {
    if truncate {
        value as i32
    }
    else {
...
pub async fn to_i32(truncate: bool, or_default: i32) {
    ...
    if truncate {
        ...
        use std::convert::TryInto;
        while let Ok(values) = value.recv_u128().await {
            check!(into.send_i32(values.into_iter().map(|val|
                TryInto::<i32>::try_into(val).unwrap_or(or_default)
            ).collect()).await)
        }
    }
}
...
/// 
#[mel_function]
pub fn to_i64(value: u128, truncate: bool, or_default: i64) -> i64 {
    if truncate {
        value as i64
    }
    else {
...
pub async fn to_i64(truncate: bool, or_default: i64) {
    ...
    if truncate {
        ...
        use std::convert::TryInto;
        while let Ok(values) = value.recv_u128().await {
            check!(into.send_i64(values.into_iter().map(|val|
                TryInto::<i64>::try_into(val).unwrap_or(or_default)
            ).collect()).await)
        }
    }
}

/// Turns `u128` into `i128`.
/// 
...
/// 
#[mel_function]
pub fn to_i128(value: u128, truncate: bool, or_default: i128) -> i128 {
    if truncate {
        value as i128
    }
    else {
        use std::convert::TryInto;
        TryInto::<i128>::try_into(value).unwrap_or(or_default)
    }
}
...
pub async fn to_i128(truncate: bool, or_default: i128) {
    ...
    if truncate {
        ...
        use std::convert::TryInto;
        while let Ok(values) = value.recv_u128().await {
            check!(into.send_i128(values.into_iter().map(|val|
                TryInto::<i128>::try_into(val).unwrap_or(or_default)
            ).collect()).await)
        }
    }
}
