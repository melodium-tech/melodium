...
/// Trigger on `byte` stream start and end.
/// 
...
/// Filter a `byte` stream according to `bool` stream.
/// 
...
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_byte(), select.recv_one_bool()) {
        if select {
...
/// Fit a stream of `byte` into a pattern.
/// 
