...
pub async fn equal() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
...
pub async fn pow() {
    while let (Ok(base), Ok(exp)) = (base.recv_one_i32().await, exponent.recv_one_i32().await) {
        check!(power.send_one_i32(base.pow(exp as u32)).await)
    }
}
...
pub async fn add() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
        check!(sum.send_one_i32(a + b).await)
    }
}
...
pub async fn div() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
        check!(quotient.send_one_i32(a / b).await)
    }
}
...
pub async fn mult() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
        check!(product.send_one_i32(a * b).await)
    }
}
...
pub async fn rem() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
        check!(remainder.send_one_i32(a % b).await)
    }
}
...
pub async fn sub() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
        check!(diff.send_one_i32(a - b).await)
    }
}
...
pub async fn min() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
        check!(min.send_one_i32(a.min(b)).await)
    }
}
...
pub async fn max() {
    while let (Ok(a), Ok(b)) = (a.recv_one_i32().await, b.recv_one_i32().await) {
        check!(max.send_one_i32(a.max(b)).await)
    }
}
...
pub async fn abs() {
    while let Ok(values) = value.recv_i32().await {
        check!(abs.send_i32(values.into_iter().map(|v| v.abs()).collect()).await)
    }
}
