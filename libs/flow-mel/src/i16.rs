...
/// Trigger on `i16` stream start and end.
/// 
...
#[mel_treatment(
    ...
    output first Block<i16>
    output last Block<i16>
)]
pub async fn trigger() {

    let mut last_value = None;

    if let Ok(values) = stream.recv_i16().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_i16(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_i16().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_i16(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<i16>
    output stream Stream<i16>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_i16().await {
        let _ = stream.send_one_i16(val).await;
    }
}
...
#[mel_treatment(
    ...
    output emit Block<i16>
)]
pub async fn emit(value: i16) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_i16(value).await;
    }
}
