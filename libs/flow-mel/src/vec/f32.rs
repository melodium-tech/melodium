...
/// ```
#[mel_treatment(
    input first Stream<Vec<f32>>
    input second Stream<Vec<f32>>
    output chained Stream<Vec<f32>>
)]
pub async fn chain() {

    while let Ok(vectors) = first.recv_vec_f32().await {

        check!(chained.send_vec_f32(vectors).await)
    }

    while let Ok(vectors) = second.recv_vec_f32().await {

        check!(chained.send_vec_f32(vectors).await)
    }
}

/// Merge two streams of `Vec<f32>`.
/// 
...
/// ```
#[mel_treatment(
    input a Stream<Vec<f32>>
    input b Stream<Vec<f32>>
...
pub async fn merge() {
    while let Ok(select) = select.recv_one_bool().await {
        let val;
        if select {
            if let Ok(v) = a.recv_one_vec_f32().await {
                val = v;
            }
            else {
                break;
            ...
            if let Ok(v) = b.recv_one_vec_f32().await {
                val = v;
            }
            else {
                break;
        ...
        check!(value.send_one_vec_f32(val).await)
    }
}

/// Filter a `Vec<f32>` stream according to `bool` stream.
/// 
...
#[mel_treatment(
    ...
    input select Stream<bool>
    output accepted Stream<Vec<f32>>
    output rejected Stream<Vec<f32>>
)]
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_vec_f32(), select.recv_one_bool()) {
        if select {
            if let Err(_) = accepted.send_one_vec_f32(value).await {
            ...
            if let Err(_) = rejected.send_one_vec_f32(value).await {
...
/// Trigger on `Vec<f32>` stream start and end.
/// 
...
/// ```
#[mel_treatment(
    input stream Stream<Vec<f32>>
    output start Block<void>
    output end Block<void>
    output first Block<Vec<f32>>
    output last Block<Vec<f32>>
)]
pub async fn trigger() {

    let mut last_value = None;

    if let Ok(values) = stream.recv_vec_f32().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_vec_f32(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_vec_f32().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_vec_f32(val).await;
    }
...
#[mel_treatment(
    ...
    output stream Stream<Vec<f32>>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_vec_f32().await {
        let _ = stream.send_one_vec_f32(val).await;
    }
}

/// Emit a block `Vec<f32>` value.
/// 
...
#[mel_treatment(
    ...
    output emit Block<Vec<f32>>
)]
pub async fn emit(value: Vec<f32>) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_vec_f32(value).await;
    }
}

/// Gives pattern of a `Vec<f32>` stream.
/// 
/// ```mermaid
/// graph LR
///     T("pattern()")
...
pub async fn pattern() {
    while let Ok(vectors) = stream.recv_vec_f32().await {
        check!(pattern.send_vec_void(vectors.into_iter().map(|vec| vec![(); vec.len()]).collect()).await)
    }
}

/// Fit a stream of `f32` into stream of `Vec<f32>`, using a pattern.
/// 
...
pub async fn fit() {
    'main: while let Ok(patterns) = pattern.recv_vec_void().await {
        for pattern in patterns {
            ...
            check!('main, fitted.send_one_vec_f32(vector).await)
        }
    }
}
...
#[mel_treatment(
    default value 0
    input pattern Stream<Vec<void>>
    output filled Stream<Vec<f32>>
)]
pub async fn fill(value: f32) {
    while let Ok(pat) = pattern.recv_vec_void().await {
        check!(filled.send_vec_f32(pat.into_iter().map(|p| vec![value.clone(); p.len()]).collect()).await)
    }
}
...
#[mel_treatment(
    ...
    output resized Stream<Vec<f32>>
)]
pub async fn resize(default: f32) {
    while let Ok(size) = size.recv_one_u64().await {
        if let Ok(mut vec) = vector.recv_one_vec_f32().await {
            vec.resize(size as usize, default.clone());
            check!(resized.send_one_vec_f32(vec).await);
        }
        else {
            break;
        }
