...
3. `/greet` parses the JSON body and rebuilds a response with `std/data/string_map::entry`/`insert` + `fromStringMap`: the whole body is echoed back as one JSON string value rather than read field by field.
