...
/// Trigger on `f32` stream start and end.
/// 
...
#[mel_treatment(
    ...
    output first Block<f32>
    output last Block<f32>
)]
pub async fn trigger() {

    let mut last_value = None;

    if let Ok(values) = stream.recv_f32().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_f32(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_f32().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_f32(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<f32>
    output stream Stream<f32>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_f32().await {
        let _ = stream.send_one_f32(val).await;
    }
}
...
#[mel_treatment(
    ...
    output emit Block<f32>
)]
pub async fn emit(value: f32) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_f32(value).await;
    }
}
