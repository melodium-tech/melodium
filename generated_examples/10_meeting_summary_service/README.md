...
**`main`** — Entry point. Starts the server, then wires each `/summarise` connection to `summariseRequest`.

**`summariseRequest[stt, llm]`** — Per-request pipeline:
1. `transcribe[stt]` receives audio bytes, returns a `Block<string>` transcript.
...
2. For each `POST /summarise` request, Mélodium creates a new **track** — fully isolated from all other concurrent requests.
...
7. Multiple simultaneous uploads each run their own independent track with their own Whisper and GPT-4o calls — no queue, no serialisation.
