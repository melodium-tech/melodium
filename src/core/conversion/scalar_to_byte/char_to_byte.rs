...
pub struct CharToByte {
    ...
    data_input_sender: Sender<char>,
    data_input_receiver: Receiver<char>,
...
impl CharToByte {
        ...
        lazy_static! {
            static ref DESCRIPTOR: Arc<CoreTreatmentDescriptor> = {
                ...
                let rc_descriptor = CoreTreatmentDescriptor::new(
                    core_identifier!("conversion";"CharToByte"),
                    models![],
                    treatment_sources![],
                    vec![],
                    vec![
    ...
    async fn to_bytes(&self) -> ResultStatus {
        ...
        while let Ok(ch) = self.data_input_receiver.recv().await {

            let output_string = ch.to_string();
            let output_data = output_string.as_bytes();

            for transmitter in &inputs_to_fill {
                match transmitter {
                    Transmitter::Byte(sender) => for byte in output_data { sender.send(*byte).await.unwrap() },
                    _ => panic!("{} sender expected!", std::any::type_name::<bool>())
                };
            }
        }
...
impl Treatment for CharToByte {
    ...
    fn get_inputs(&self) -> HashMap<String, Vec<Transmitter>> {
        ...
        hashmap.insert("value".to_string(), vec![Transmitter::Char(self.data_input_sender.clone())]);
