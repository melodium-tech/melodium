...
proc.stdout ──▶ decode ──▶ writeTextLocal
proc.stderr ──▶ decode ──▶ logErrors
...
3. Both `stdout` and `stderr` are decoded and logged/written directly; a successful run with nothing on stderr produces no items on `stderrText.text` at all, so `stderrLog` logs nothing, not an empty line.

### Key Mélodium patterns used

- **A subprocess is just another treatment with stream ports**: `stdin`/`stdout`/`stderr` are `Stream<byte>` like any HTTP body or file content; every idiom from earlier examples (decode, encode) applies unchanged.
