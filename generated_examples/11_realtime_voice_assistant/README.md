...
**`main` entrypoint** — local STT + remote LLM (recommended for production):
...
**`localOnly` entrypoint** — fully offline:
...
- Requires ~14 GB of RAM for Mistral 7B.
...
1. **`main`**: Only Whisper is fetched and loaded. Recording starts as soon as `loadAsr.loaded` fires. Each transcribed segment immediately triggers a new `llmStream` call; tokens appear on the console token by token before the sentence is complete.
