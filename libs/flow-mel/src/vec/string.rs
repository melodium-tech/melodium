...
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_vec_string(), select.recv_one_bool()) {
        if select {
...
pub async fn fill(value: string) {
    while let Ok(pat) = pattern.recv_vec_void().await {
        check!(filled.send_vec_string(pat.into_iter().map(|p| vec![value.clone(); p.len()]).collect()).await)
    }
}
