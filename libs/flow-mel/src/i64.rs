...
/// ```
#[mel_treatment(
    input first Stream<i64>
    input second Stream<i64>
    output chained Stream<i64>
)]
pub async fn chain() {
    while let Ok(values) = first.recv_i64().await {
        check!(chained.send_i64(values).await)
    }

    while let Ok(values) = second.recv_i64().await {
        check!(chained.send_i64(values).await)
    }
}

/// Trigger on `i64` stream start and end.
///
...
#[mel_treatment(
    ...
    output first Block<i64>
    output last Block<i64>
)]
pub async fn trigger() {
    let mut last_value = None;

    if let Ok(values) = stream.recv_i64().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_i64(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_i64().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_i64(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<i64>
    output stream Stream<i64>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_i64().await {
        let _ = stream.send_one_i64(val).await;
    }
}

/// Merge two streams of `i64`.
///
...
pub async fn merge() {
    while let Ok(select) = select.recv_one_bool().await {
        let val;
        if select {
            if let Ok(v) = a.recv_one_i64().await {
                val = v;
            } else {
                break;
            }
        } else {
            if let Ok(v) = b.recv_one_i64().await {
                val = v;
            } else {
                break;
            }
        ...
        check!(value.send_one_i64(val).await)
    }
}
...
#[mel_treatment(
    ...
    output filled Stream<i64>
)]
pub async fn fill(value: i64) {
    while let Ok(pat) = pattern.recv_void().await {
        check!(filled.send_i64(vec![value.clone(); pat.len()]).await)
    }
}

/// Filter a `i64` stream according to `bool` stream.
///
...
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_i64(), select.recv_one_bool())
    {
        if select {
            if let Err(_) = accepted.send_one_i64(value).await {
            ...
            if let Err(_) = rejected.send_one_i64(value).await {
...
#[mel_treatment(
    ...
    output emit Block<i64>
)]
pub async fn emit(value: i64) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_i64(value).await;
    }
}
