...
**`main`** is the entry point. It fetches both models in parallel (`startup.trigger` fans out to `fetchAsr` and `fetchLlm`). Each fetch drives its own `load`. Recording and decoding begin once Whisper is ready; `promptLlm` is called for each transcribed segment.

**`promptLlm[llm: Mistral]`** wraps each question string in a Mistral instruction template using `entry` + `format`, then calls `generate[mistral=llm]`. It outputs the generated text as a stream of strings.
...
- **Fully local execution**: after initial download, the pipeline runs entirely on-device with no API calls, no network latency, and no usage costs.
