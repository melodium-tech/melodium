...
Most tutorial examples were checked with `melodium check` and actually run with `melodium run`, real logging included: see each README's *Runtime behaviour* section for specifics. If you hit something that looks wrong while adapting these examples, run them with real logging before assuming your graph is correct: `melodium check` only verifies types, not runtime behaviour.
