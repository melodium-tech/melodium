...
3. A second, tiny JS call, `pickTier(decision)`, projects just the `tier` field back out: chaining a second `process` call on the first call's own output reads one field out of a result already computed in JS.
