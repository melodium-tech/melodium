...
use core::fmt::{Debug};
use melodium_common::descriptor::{Designer, Collection, Parameterized, Model as ModelDescriptor, Parameter as ParameterDescriptor, Variability};
...
impl ModelInstanciation {
    pub fn new(host_treatment: &Arc<RwLock<Treatment>>, descriptor: &Arc<dyn ModelDescriptor>, name: &str) -> Self {
        Self {
    ...
    pub fn validate(&self) -> Result<(), LogicError> {
        ...
        if let Some(_forbidden_var) = self.parameters.iter().find(|&(param_name, param)| *param.read().unwrap().parent_descriptor().upgrade().unwrap().parameters().get(param_name).unwrap().variability() != Variability::Const) {
