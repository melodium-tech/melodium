...
impl AssignedModel {
    ...
    /// let borrowed_treatment_instanciation = borrowed_treatment.find_treatment_instanciation("Decoder").unwrap().read().unwrap();
    /// let borrowed_assigned_model = borrowed_treatment_instanciation.find_assigned_model("AudioManager").unwrap().read().unwrap();
    ///
