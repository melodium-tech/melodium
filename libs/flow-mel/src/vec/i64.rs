...
pub async fn flatten() {
    'main: while let Ok(vectors) = vector.recv_vec_i64().await {
        for vec in vectors {
            check!('main, value.send_i64(vec).await)
        }
    }
}

/// Chain two streams of `Vec<i64>`.
/// 
/// 
/// ```mermaid
/// graph LR
...
/// ```
#[mel_treatment(
    input first Stream<Vec<i64>>
    input second Stream<Vec<i64>>
    output chained Stream<Vec<i64>>
)]
pub async fn chain() {

    while let Ok(vectors) = first.recv_vec_i64().await {

        check!(chained.send_vec_i64(vectors).await)
    }

    while let Ok(vectors) = second.recv_vec_i64().await {

        check!(chained.send_vec_i64(vectors).await)
    }
}

/// Merge two streams of `Vec<i64>`.
/// 
...
/// ```
#[mel_treatment(
    input a Stream<Vec<i64>>
    input b Stream<Vec<i64>>
...
pub async fn merge() {
    while let Ok(select) = select.recv_one_bool().await {
        let val;
        if select {
            if let Ok(v) = a.recv_one_vec_i64().await {
                val = v;
            }
            else {
                break;
            ...
            if let Ok(v) = b.recv_one_vec_i64().await {
                val = v;
            }
            else {
                break;
        ...
        check!(value.send_one_vec_i64(val).await)
    }
}

/// Filter a `Vec<i64>` stream according to `bool` stream.
/// 
...
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_vec_i64(), select.recv_one_bool()) {
        if select {
            if let Err(_) = accepted.send_one_vec_i64(value).await {
            ...
            if let Err(_) = rejected.send_one_vec_i64(value).await {
...
#[mel_treatment(
    default value 0
    input pattern Stream<Vec<void>>
    output filled Stream<Vec<i64>>
)]
pub async fn fill(value: i64) {
    while let Ok(pat) = pattern.recv_vec_void().await {
        check!(filled.send_vec_i64(pat.into_iter().map(|p| vec![value.clone(); p.len()]).collect()).await)
    }
}
...
#[mel_treatment(
    ...
    output resized Stream<Vec<i64>>
)]
pub async fn resize(default: i64) {
    while let Ok(size) = size.recv_one_u64().await {
        if let Ok(mut vec) = vector.recv_one_vec_i64().await {
            vec.resize(size as usize, default.clone());
            check!(resized.send_one_vec_i64(vec).await);
        }
        else {
            break;
        }
