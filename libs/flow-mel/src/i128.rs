...
/// ```
#[mel_treatment(
    input first Stream<i128>
    input second Stream<i128>
    output chained Stream<i128>
)]
pub async fn chain() {

    while let Ok(values) = first.recv_i128().await {

        check!(chained.send_i128(values).await)
    }

    while let Ok(values) = second.recv_i128().await {

        check!(chained.send_i128(values).await)
    }
}

/// Trigger on `i128` stream start and end.
/// 
...
#[mel_treatment(
    ...
    output first Block<i128>
    output last Block<i128>
)]
pub async fn trigger() {

    let mut last_value = None;

    if let Ok(values) = stream.recv_i128().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_i128(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_i128().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_i128(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<i128>
    output stream Stream<i128>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_i128().await {
        let _ = stream.send_one_i128(val).await;
    }
}

/// Merge two streams of `i128`.
/// 
...
pub async fn merge() {
    while let Ok(select) = select.recv_one_bool().await {
        let val;
        if select {
            if let Ok(v) = a.recv_one_i128().await {
                val = v;
            }
            else {
                break;
            ...
            if let Ok(v) = b.recv_one_i128().await {
                val = v;
            }
            else {
                break;
        ...
        check!(value.send_one_i128(val).await)
    }
}
...
#[mel_treatment(
    ...
    output filled Stream<i128>
)]
pub async fn fill(value: i128) {
    while let Ok(pat) = pattern.recv_void().await {
        check!(filled.send_i128(vec![value.clone(); pat.len()]).await)
    }
}

/// Filter a `i128` stream according to `bool` stream.
/// 
...
#[mel_treatment(
    ...
    input select Stream<bool>
    output accepted Stream<i128>
    output rejected Stream<i128>
)]
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_i128(), select.recv_one_bool()) {
        if select {
            if let Err(_) = accepted.send_one_i128(value).await {
            ...
            if let Err(_) = rejected.send_one_i128(value).await {
...
/// Fit a stream of `i128` into a pattern.
/// 
...
#[mel_treatment(
    ...
    output emit Block<i128>
)]
pub async fn emit(value: i128) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_i128(value).await;
    }
}
