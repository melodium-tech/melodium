...
**`main`** is the entry point. It starts the HTTP server and wires each incoming connection to the `chat` treatment.

**`chat`** is the per-request pipeline. It decodes the raw request bytes to text, calls the remote LLM using `llmStream`, and encodes the token stream back to bytes for the HTTP response. LLM errors are forwarded to `logErrors`.
...
- **`trigger<byte>()`** separates the incoming byte stream arrival from the trigger that sends headers and status, allowing status 200 to be emitted before the body is fully read.
