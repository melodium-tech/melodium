...
- The JS function (`transform`) adds a letter grade (`A` to `F`) based on a `score` field.
...
**`main`** is the entry point. It starts the server, logs readiness, and wires each `/transform` connection to `jsTransform`.

**`jsTransform`** is the per-request pipeline:
1. `decode` converts bytes to a UTF-8 string.
2. `toJson` parses the string into `Option<Json>`.
...
7. `encode` converts the string back to bytes for the HTTP response.
...
- **`unwrap<Json>()` vs `unwrapOr<Json>()`**: `unwrap` is used before `process` (malformed JSON is a hard error), while `unwrapOr` is used after `process` (JS errors yield a graceful null fallback).
