...
/// ```
#[mel_treatment(
    input first Stream<u8>
    input second Stream<u8>
    output chained Stream<u8>
)]
pub async fn chain() {
    while let Ok(values) = first.recv_u8().await {
        check!(chained.send_u8(values).await)
    }

    while let Ok(values) = second.recv_u8().await {
        check!(chained.send_u8(values).await)
    }
}

/// Trigger on `u8` stream start and end.
///
...
#[mel_treatment(
    ...
    output first Block<u8>
    output last Block<u8>
)]
pub async fn trigger() {
    let mut last_value = None;

    if let Ok(values) = stream.recv_u8().await {
        let _ = start.send_one_void(()).await;
        if let Some(val) = values.first().cloned() {
            let _ = first.send_one_u8(val).await;
        }
        last_value = values.last().cloned();
        let _ = futures::join!(start.close(), first.close());
    }

    while let Ok(values) = stream.recv_u8().await {
        last_value = values.last().cloned();
    }

    let _ = end.send_one_void(()).await;
    if let Some(val) = last_value {
        let _ = last.send_one_u8(val).await;
    }
...
/// ```
#[mel_treatment(
    input block Block<u8>
    output stream Stream<u8>
)]
pub async fn stream() {
    if let Ok(val) = block.recv_one_u8().await {
        let _ = stream.send_one_u8(val).await;
    }
}
...
pub async fn merge() {
    while let Ok(select) = select.recv_one_bool().await {
        let val;
        if select {
            if let Ok(v) = a.recv_one_u8().await {
                val = v;
            } else {
                break;
            }
        } else {
            if let Ok(v) = b.recv_one_u8().await {
                val = v;
            } else {
                break;
            }
        ...
        check!(value.send_one_u8(val).await)
    }
}
...
#[mel_treatment(
    ...
    output filled Stream<u8>
)]
pub async fn fill(value: u8) {
    while let Ok(pat) = pattern.recv_void().await {
        check!(filled.send_u8(vec![value.clone(); pat.len()]).await)
    }
}

/// Filter a `u8` stream according to `bool` stream.
///
...
pub async fn filter() {
    ...
    while let (Ok(value), Ok(select)) = futures::join!(value.recv_one_u8(), select.recv_one_bool())
    {
        if select {
            if let Err(_) = accepted.send_one_u8(value).await {
            ...
            if let Err(_) = rejected.send_one_u8(value).await {
...
/// Fit a stream of `u8` into a pattern.
///
...
#[mel_treatment(
    ...
    output emit Block<u8>
)]
pub async fn emit(value: u8) {
    if let Ok(_) = trigger.recv_one_void().await {
        let _ = emit.send_one_u8(value).await;
    }
}
